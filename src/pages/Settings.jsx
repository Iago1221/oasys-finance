...
const Settings = () => {
    ...
    // Estados para as integrações (inicializando do localStorage se existir)
    const [isBankingActive, setIsBankingActive] = useState(() => {
        const saved = localStorage.getItem('isBankingActive');
        return saved !== null ? JSON.parse(saved) : true;
    });
    const [isLogisticsActive, setIsLogisticsActive] = useState(() => {
        const saved = localStorage.getItem('isLogisticsActive');
        return saved !== null ? JSON.parse(saved) : false;
    });
    const [isCRMActive, setIsCRMActive] = useState(() => {
    ...
    // Efeito para persistir integrações
    useEffect(() => {
        localStorage.setItem('isBankingActive', JSON.stringify(isBankingActive));
    }, [isBankingActive]);

    useEffect(() => {
        localStorage.setItem('isLogisticsActive', JSON.stringify(isLogisticsActive));
    }, [isLogisticsActive]);

    useEffect(() => {
        localStorage.setItem('isCRMActive', JSON.stringify(isCRMActive));
    }, [isCRMActive]);
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 antialiased min-h-screen pb-24 transition-colors duration-300">
